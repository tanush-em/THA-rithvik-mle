...
source_url: "https://support.devplatform.com/articles/8826242281-database-engineer-questions"
...
1.   In the **Problem details** section:
    ...
    2.  Describe the problem in the **Problem description** field. You can use the formatting menu to format the text or to include elements such as tables or images.
        ...
        **Note:** 
        ...
        - Click **Use content templates** and then select **DevPlatform’s Database Question Template** to insert a predefined template.
...
1.  Select one or more databases for the question.
...
3.  In the **Expected Output** section:
    ...
    3.  (Optional) Click the settings icon and select one or more of the following options:
        ...
        - **Allow DB write permissions to candidates:** Allow candidates to run write operations such as `INSERT`, `UPDATE`, or `DELETE`.
...
DevPlatform scores Database questions automatically by comparing the candidate’s query result with the expected output.
...
When creating the question, you can choose whether scoring considers the row order. For more information, see [Step 2: Database Setup](https://support.devplatform.com/articles/8826242281-database-engineer-questions#step-2-database-setup).

If the query logic is correct but the output differs slightly, review the response manually. You can update the score from the candidate’s **Detailed Report** to award partial credit. For more information, see [📄 Viewing a Candidate's Detailed Test Report](/articles/8263794320).
