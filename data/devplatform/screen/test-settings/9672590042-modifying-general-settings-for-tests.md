---
title: "Modify General Settings for Tests"
title_slug: "modify-general-settings-for-tests"
source_url: "https://support.devplatform.com/articles/9672590042-modifying-general-settings-for-tests"
article_slug: "9672590042-modifying-general-settings-for-tests"
last_updated_exact: "Jul 30, 2025, 9:49 AM"
last_updated_relative: "Last updated 9 months ago"
breadcrumbs:
  - "Screen"
  - "Test Settings"
---

# Modify General Settings for Tests

_Last updated: Jul 30, 2025, 9:49 AM (Last updated 9 months ago)_

DevPlatform allows you to modify the general settings of a test such as key metadata and scheduling options. Most of these settings are defined when you initially create the test. 

## Prerequisites 

You must have already created a test.

## Modifying general settings

To modify the general settings of a test:

1.  Log in to your **DevPlatform for Work** account using your credentials.

2.  Go to the **Tests** tab.

3.  Open the test you want to update.

4.  Go to **Settings \> General.**\
    \

    ![image.png](https://assets.usepylon.com/e6a58e21-be80-4777-9eaf-f73beeee94d9%2F1753693496158-image.png?Expires=253370764800&Signature=ilTIdaKGYfWrEGqAKslNefJ0vkyIGaqqPcMR5GprvUVfKHCnclW~trdCFBK8lrNouUtvzLRci681ES9cHuuc4jMkI1zj3eWEZQSUy97Xa-beYQ8K6mrFUCMHk5D8~vyFG5TKZrg7Vkq8xG49t8ooxsCySAG7n4eXC1BxvOCk1elYavPbd~YIZ0gIGNue1c178Z0UMre7meOm8MeFskUKQVS0dQxg6geR1IhOEMYRXImTsQ6r-drxbTcXmLkAs4B2JpUsSWkebZD0X3tRcXdJLMM9ddtc8~N5Gr7JvdQnVotbybNCmlJJowDiipS96PEbr1ruz8rffDDuXm-tcb7YRA__&Key-Pair-Id=K3NV4LZ47N8M46)

    \
    In the **General** section, you can update the following:

    - **Job description link**: Add a link to the job description. This helps hiring stakeholders and reviewers contextualize the test.

    - **Role association**: Select the role associated with the test (for example, Software Engineer). This helps categorize tests by job function.

    - **Work experience**: Tag the test to an experience level (for example, 0–2 years). This helps align the test with candidate seniority.

    - **Test label**: Enter a custom label. This label appears in the test listing view and helps you identify tests easily.

    - **Candidate-facing language**: Select the language in which the test interface appears to candidates. The available options are English and Canadian French. For more information on changing to Canadian French, see [📄 Assess Candidates in Canadian French Language](/articles/6201558176).

    - **Test expiration time**: Define a start and end time (including time zone) for when the test remains active. Candidates can only take the test during this window.

<!-- -->

1.  Click **Save** **Changes** to apply your changes.
