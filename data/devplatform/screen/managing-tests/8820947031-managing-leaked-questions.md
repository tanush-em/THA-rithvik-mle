...
source_url: "https://support.devplatform.com/articles/8820947031-managing-leaked-questions"
...
A question is considered leaked if it appears in publicly accessible search results and matches a DevPlatform Library or customer-created question both structurally and semantically.

# Detecting leaked questions

DevPlatform uses a leak detection algorithm that automatically searches at regular intervals to identify leaked questions from the leak-protected skill sets in your tests. 
...
- Although the leak detection algorithm is highly accurate, DevPlatform recommends that you conduct your own validation before taking action on flagged content.
...
If you find a DevPlatform Library or your company question that appears online, you can manually report it.
...
3.  Select the **Tests** or **Interview** tab.

4.  Select the source of questions from the **Library** drop-down.

5.  Locate the leaked question.
...
By default, DevPlatform hides leaked questions from the Library.

To view leaked questions:

1.  Go to the **Library** tab.

2.  Select the **Tests** or **Interview** tab.

3.  Select the source of questions from the **Library** drop-down.
...
- **Monitor ongoing tests**: If a leaked question is included in an active test, use the [plagiarism detection](https://support.devplatform.com/articles/8000786908-ai-plagiarism-detection) functionality to monitor submissions. Replace the question if required.
...
To maintain content integrity, DevPlatform takes the following measures:

- **DMCA takedowns**: DevPlatform issues DMCA takedown notices to websites hosting leaked questions, prioritizing cases based on severity and domain type. For more information, see [DevPlatform's DMCA Process](https://www.devplatform.com/blog/dmca-takedown-policy/)**.**

- **Library updates**: DevPlatform runs leakage detection daily and updates the question library regularly with new question types and skills. For more information, see [📄 DevPlatform Question Count Comparison by Subscription Plan](/articles/6095274436).

- **Automatic replacement of leaked questions in tests:** If a test contains leaked questions, DevPlatform automatically replaces them with similar questions for leakage-protected sections.
...
- **Custom question monitoring:** DevPlatform runs leakage detection for customer-created questions every two weeks to identify and mitigate potential breaches.
