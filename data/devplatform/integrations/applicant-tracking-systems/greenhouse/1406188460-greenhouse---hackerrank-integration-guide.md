---
title: "Greenhouse - DevPlatform Integration Guide Prerequisites Integrating Greenhouse with DevPlatform Glossary Frequently Asked Questions (FAQs)"
title_slug: "greenhouse-devplatform-integration-guide-prerequisites-integrating-greenhouse-with-devplatform-glossary-frequently-asked-questions-faqs"
source_url: "https://support.devplatform.com/articles/1406188460-greenhouse---devplatform-integration-guide"
article_slug: "1406188460-greenhouse---devplatform-integration-guide"
...
# Greenhouse - DevPlatform Integration Guide Prerequisites Integrating Greenhouse with DevPlatform Glossary Frequently Asked Questions (FAQs)

_Last updated: Feb 16, 2026, 1:06 PM (Last updated 2 months ago)_

DevPlatform integrates with Greenhouse to streamline the candidate screening process for recruiters.\
This guide explains how to integrate Greenhouse with DevPlatform for Work, allowing you to send tests, schedule interviews, and view results seamlessly.

# Prerequisites

Before you begin, ensure you meet the following requirements:

- You have admin access to your DevPlatform and Greenhouse accounts.

- Your organization has an active Pro or Enterprise plan with DevPlatform.

- Your DevPlatform login email address matches the email address associated with your Greenhouse account.

# Integrating Greenhouse with DevPlatform

To integrate Greenhouse with DevPlatform:

**Step 1: Generate an integration API token in DevPlatform**

The integration [API token](https://support.devplatform.com/articles/1406188460-greenhouse---devplatform-integration-guide#glossary-11) allows Greenhouse to connect securely with your DevPlatform account.

1.  Log in to your DevPlatform account using your credentials.

2.  Go to **Settings \> Integrations \> Greenhouse \> Connect,** or open the [Greenhouse integration](https://www.devplatform.com/work/settings/integrations/greenhouse/configuration) page directly.
...
**Step 2: Add the DevPlatform API token in Greenhouse**

This step authorizes Greenhouse to connect with your DevPlatform account.

1.  Log in to your Greenhouse account using your credentials.

2.  Go to **Integrations \> DevPlatform**, or open the [DevPlatform Integration](https://app.greenhouse.io/partners?partner_type=integration&partner_id=45) page directly.
...
The [Harvest API](https://support.devplatform.com/articles/1406188460-greenhouse---devplatform-integration-guide#glossary-11) allows Greenhouse to securely share candidate and job information with DevPlatform. It also allows automatic syncing of test results and interview information.
...
3.  In the **Create new credential** dialog box, set the following options:
    ...
    - Select **DevPlatform** as the Partner.
...
5.  Click **Copy** to copy the API key to your clipboard.
    ...
    **Note:** You need this token in Step 5: Add the Harvest API key in DevPlatform.

6.  Click **I have stored the API key** to confirm that you have saved it securely.

7.  In the **Manage permissions** section, select the required permissions listed below for the DevPlatform Integration.
    ...
    - **Candidates**
      ...
      - `Patch: Edit Candidate`: Updates candidate profiles with DevPlatform data such as test results, integrity signals, interview feedback. The system stores this data in custom fields or activity.

      - `Post: Add Candidate`: Create a new candidate in Greenhouse when you create a candidate in DevPlatform. This ensures the candidate appears in Greenhouse and can receive notes, attachments, and tags.
      ...
      - `Post: Add Attachment`: Attach the candidate’s resume to their Greenhouse profile when you create the candidate from DevPlatform.
    ...
    - **Custom fields**
      ...
      - `Patch: Update Custom Field`:\
        Update DevPlatform data to your custom fields. For example, store test results, integrity signals, and interview feedback.

      - `Post: Create Custom Field`:\
        Create a new custom field in Greenhouse if a required field for DevPlatform data does not exist. This ensures the integration stores results and feedback correctly.
    ...
    - **Tags**
      ...
      - `Get: List Candidate Tags`: Retrieve existing tag names in Greenhouse. This ensures the integration adds only missing DevPlatform tags, such as test status tags.
      ...
      - `Post: Add New Candidate Tag`: Create new tag names in Greenhouse, such as DevPlatform test status tags, if they do not already exist.

      - `Put: Add Candidate Tag`: Apply the appropriate tag to a candidate after the candidate is created or updated from DevPlatform.
...
The [Admin User ID](https://support.devplatform.com/articles/1406188460-greenhouse---devplatform-integration-guide#glossary-11) identifies the Greenhouse user who owns the integration.
...
**Step 5: Add the Harvest API key and admin user ID in DevPlatform**

This step completes the data link between DevPlatform and Greenhouse.

1.  In the DevPlatform account, go to **Settings \> Integrations \> Greenhouse \> Connect,** or open the [Greenhouse integration](https://www.devplatform.com/work/settings/integrations/greenhouse/configuration) page directly.
...
7.  Create a ticket:
    ...
    2.  Enter `GreenHouse DevPlatform Integration API` as **Subject line**.

    3.  Enter `Request to securely send the DevPlatform Greenhouse API key for the Greenhouse – DevPlatform integration.`** **as **Summary of your request**. 
...
1.  Go to **Integrations \> DevPlatform**, or open the [DevPlatform Integration](https://app.greenhouse.io/partners?partner_type=integration&partner_id=45) page directly in your Greenhouse account.
...
After you receive confirmation that the token is saved, the Greenhouse-DevPlatform integration is complete. This process is usually completed within 24 hours.

**Step 7 (Optional): Add a user to the Greenhouse - DevPlatform Integration**

After you complete the Greenhouse-DevPlatform integration, you can let other users integrate Greenhouse with DevPlatform using the same API key.
...
5.  Ask the user to follow Step 2: Add the DevPlatform API token in Greenhouse to add the API key to their Greenhouse account.

# Glossary

- **Harvest API:** Greenhouse’s data-sharing interface that connects partner applications such as DevPlatform.
...
- **API Token:** A secure key that allows Greenhouse to recognize and authenticate DevPlatform’s connection request.

# Frequently Asked Questions (FAQs)

**Why is the Greenhouse Integration page not showing in my DevPlatform account?**

You do not have admin access to your DevPlatform account. Only admins can access the integration settings page.

**Why don’t I see the ATS Settings updated message after I click Test and Save?**

Verify that you entered the correct Greenhouse User ID and Harvest API Token in DevPlatform.

**Why am I the only user who can’t send tests through Greenhouse?**

 Check the following:

- Your DevPlatform and Greenhouse login email addresses match.

- You have a Recruiter or Company Admin access in DevPlatform.

- You have **Send Test** permissions in DevPlatform.
