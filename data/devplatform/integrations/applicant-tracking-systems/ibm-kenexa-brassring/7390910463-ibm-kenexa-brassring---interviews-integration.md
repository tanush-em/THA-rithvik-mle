...
source_url: "https://support.devplatform.com/articles/7390910463-ibm-kenexa-brassring---interviews-integration"
...
The below diagram describes the workflow of the BrassRing and DevPlatform Interviews integration for recruiters. Recruiters will move a candidate into a custom DevPlatform Interview status. Upon entering this status, BrassRing will dispatch a payload to DevPlatform via the Candidate Export API. DevPlatform will then generate a new Interview URL and Scheduling link and respond back to BrassRing. The Recruiter can use the Interview URL directly in their own scheduling tool. Optionally, they can access the scheduling link to set up a date and time within DevPlatform. DevPlatform will then send out the calendar invitation automatically. After the Interview has occurred, a link to the Interview report will be sent back to BrassRing via the Form Import API.
...
PROD: [https://ats.devplatform.com/brassring-codepair/v3/schedule-codepair](https://ats.devplatform.com/brassring/v1/schedule-codepair)

STAGE: [https://ats.devplatform.com/brassring-codepair-stage/v3/schedule-codepair](https://ats.devplatform.com/brassring/v1/schedule-codepair)
...
| EMAIL | Varchar | This is the candidate email address. DevPlatform needs this to tie the interview to the candidate |
...
<form formTypeId="376" formName="DevPlatform_CodePair_Results" formId="" action="Insert" resumeKey="" FirstName="" LastName="" email="" homePhone="" language="EN" autoreq="" optionalreq="">
...
Once the XML payload is created, you must send a preview XML to DevPlatform so the correct identifiers can be sent from DevPlatform for Work.
