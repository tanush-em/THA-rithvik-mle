---
title: "IBM Kenexa BrassRing - DevPlatform Tests Integration"
title_slug: "ibm-kenexa-brassring-devplatform-tests-integration"
source_url: "https://support.devplatform.com/articles/7795424803-ibm-kenexa-brassring---devplatform-tests-integration"
article_slug: "7795424803-ibm-kenexa-brassring---devplatform-tests-integration"
...
# IBM Kenexa BrassRing - DevPlatform Tests Integration

_Last updated: Dec 28, 2024, 4:09 AM (Last updated 1 year ago)_

## Overview

The below diagram describes the workflow of the BrassRing and DevPlatform integration for recruiters. Recruiters will move a candidate into a custom DevPlatform Test status. Upon entering this status, BrassRing will dispatch a payload to DevPlatform via the candidate Export API. DevPlatform will then send a test invitation to the candidate. After the candidate has completed the test and DevPlatform has graded the test, DevPlatform will send the results back to BrassRing via the Form Import API. 
...
<li><p>Recommended: DevPlatform for Work Sandbox account for integration testing.</p></li>
</ul></td>
<td data-colspan="1" data-rowspan="1" style="width: 332px"><ul>
...
- Configure the List of DevPlatform Tests in BrassRing
...
The lists of tests need to be set up on the BrassRing side as options in a DEVPLATFORM_TEST_ID field. 
...
When building out and testing the integration for the first time, you can get the Name and Test ID from the DevPlatform Test page and the page URL, respectively. In the example below, the Test ID is 454785 and the Test Name is “DevPlatform Full-stack Developer Hiring Test.”

When the integration is ready, your DevPlatform Solutions Engineer can do a one time bulk export of all Test Names and Test IDs to be entered into BrassRing.

Any time new tests are created in DevPlatform that need to be used in BrassRing, you will need to add them in manually. 

### Configure the Candidate Export 

#### **Obtain the BrassRing API key from DevPlatform for Work**
...
[https://ats.devplatform.com/brassring/v2/invite](https://ats.devplatform.com/brassring/v2/invite)

The username for this export should be set to the BrassRing API key generated within DevPlatform. (The password field is not used by DevPlatform and can be set to any value if it is a mandatory field.) 
...
This is the format for the [Candidate Export XML](http://media.kenexa.com/Training/TSBR/Outbound/IBM_Kenexa_BrassRing_on_Cloud-Candidate_Export_Technical_Specifications_API.docx) that is understood by DevPlatform out of the box. You will need to configure Workbench to create a matching XML:
...
      <PacketId>1</PacketId>
      <Action>SET</Action>
      <Manifest>DEVPLATFORM_CANDIDATE</Manifest>
    </PacketInfo>
    <Payload><![CDATA[<?xml version="1.0"?>
<DEVPLATFORM_CANDIDATE>
...
      <PacketId>2</PacketId>
      <Action>SET</Action>
      <Manifest>DEVPLATFORM_CANDIDATE_REQEXPORT</Manifest>
    </PacketInfo>
    <Payload><![CDATA[<DEVPLATFORM_CANDIDATE_REQEXPORT language="en">
        <REQFORM id="111"></REQFORM>
 <RECRUITEREMAIL></RECRUITEREMAIL>
 <DEVPLATFORMTESTID></DEVPLATFORMTESTID>
</DEVPLATFORM_CANDIDATE_REQEXPORT>
]]></Payload>
  </Packet>
</Envelope>
```
...
|----|----|----|
| Field Name | Format | Note |
| EMAIL | Varchar | This is the candidate email address. DevPlatform will send the invitation email.  |
...
|----|----|----|
| Field Name | Format | Note |
| RECRUITEREMAIL | Varchar | This is the email address of the user in BrassRing who is taking the action. This is important so that the report from DevPlatform is sent to the right recruiter. Additionally, this ensures correct auditing in DevPlatform.  |
| DEVPLATFORMTESTID | Varchar | This is the Test ID of the test that should be sent to the candidate. For example, the Test ID for the following test URL is 454785: [https://www.devplatform.com/work/tests/454785/questions](https://www.devplatform.com/work/tests/454785/questions) |

#### **Send Candidate Export XML Payload Preview to DevPlatform **

Once the Workbench configuration is complete, generate the sample XML and send to DevPlatform for validation.

### Configure the Form Import 

#### **Setup Candidate Export XML Payload**

This is the format for the [Form Import XML](https://www.ibm.com/links?url=http%3A%2F%2Fmedia.kenexa.com%2FTraining%2FTSBR%2FInbound%2FIBM_Kenexa_BrassRing_on_Cloud-Form_Import_Technical_Specifications_API_update.docx) that is sent from DevPlatform out of the box. You will need to configure Workbench to create a matching XML:
...
      <PacketId>1</PacketId>
      <Action>SET</Action>
      <Manifest>DEVPLATFORM_FORM_IMPORT</Manifest>
    </PacketInfo>
    <Payload><![CDATA[<?xml version="1.0"?>
<form formTypeId="(generated by BR)" formName="DevPlatform Results" formId="" action="UpdateIndividual" resumeKey="" FirstName="" LastName="" email="" homePhone="" language="EN" autoreq="" optionalreq="">
...
This is the status of the candidate’s results. It matches the status that is shown in DevPlatform.  
...
<td data-colspan="1" data-rowspan="1" style="height: 44px; width: 50%"><p>This is the candidate email address. DevPlatform will send the invitation email. </p></td>
</tr>
<tr>
...
<td data-colspan="1" data-rowspan="1" style="height: 44px; width: 50%"><p>This is a link back to the test results in DevPlatform</p></td>
</tr>
<tr>
...
#### **Send Form Import XML Payload Preview to DevPlatform**
...
Once the XML payload is created, you must send a preview XML to DevPlatform so the correct identifiers can be send from DevPlatform for Work.
