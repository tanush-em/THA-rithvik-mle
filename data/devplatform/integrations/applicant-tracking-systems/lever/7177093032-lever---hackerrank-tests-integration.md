---
title: "Lever - DevPlatform Tests Integration"
title_slug: "lever-devplatform-tests-integration"
source_url: "https://support.devplatform.com/articles/7177093032-lever---devplatform-tests-integration"
article_slug: "7177093032-lever---devplatform-tests-integration"
...
# Lever - DevPlatform Tests Integration

_Last updated: Jan 23, 2025, 4:04 AM (Last updated 1 year ago)_

## Overview

The DevPlatform Tests - Lever integration enables Lever users to directly invite their candidates to different DevPlatform Tests at the appropriate stages of the job interview process. After candidates attempt DevPlatform's Tests, their scores and performance data are automatically updated in their respective profiles in Lever. The ability to leverage DevPlatform's Tests for candidate skills assessment facilitates a seamless and efficient candidate screening process in Lever.

This article provides you with detailed integration steps and also describes how you can use the integration to [send DevPlatform Test invites from Lever.](https://support.devplatform.com/articles/7177093032-lever---devplatform-tests-integration#sending-devplatform-test-invites-from-candidate-profiles-in-lever-21)
...
<tbody>
<tr>
<td data-colspan="1" data-rowspan="1" style="background-color: #eafcef; width: 50%"><p><strong>For DevPlatform for Work</strong></p></td>
...
<li><p>Log in using the same <strong>Company Admin</strong> user credentials to create and publish DevPlatform Tests, which must be accessible in Lever.</p></li>
</ul></td>
...
### Obtaining the Lever API key from DevPlatform for Work
...
2.  Scroll down on the page to the **Interviewing** section, and enable the **DevPlatform - Tests** option. 

![DevPlatform_Tests_option_in_Lever.jpg](https://assets.usepylon.com/e6a58e21-be80-4777-9eaf-f73beeee94d9%2F1734047100303-?Expires=253370764800&Signature=Z2Qo0aI8jTOMxedGmYTqb3Bh0kKRf4BJVqNpfr73uTu7ClYmaKo3nuyWso8DJVJIOAGSPtSoHdnLKdqCUW9pTYv-Z5--WpJHSoEdozY9EzXl9s-jiVqucKDOygVfl7oq4csCH5571MYMTRyCz1fZgXDYad6Q8KNG7TXdwzGdAtb~jequl4KGlNJ9giKwim4FNp1enhmcynMqoA4gF-m~tIPbil39GCjjVlGAUxndaFbYM2QDf2TlpSYpgHACwhs6nfJO7JNCAG7X7Pnq0Xtw80mRWRBOxZCdJgwBuemmJABFBFaZljoIHboCxMUzVjHLsYMYsYiuo2O9GPZsOV5ohw__&Key-Pair-Id=K3NV4LZ47N8M46)

*Enabling the "DevPlatform Tests" integration in Lever*

1.  In the **Access token** field, add the API key obtained from DevPlatform for Work.

2.  Click on the **Verify** button to test the authentication request between Lever and DevPlatform. Verification should be successful.
...
*Verifying the authentication request from Lever to DevPlatform Tests*

### Create Tests in DevPlatform for Work

1.  Log in to DevPlatform for Work using the same Company Admin user account which was used for generating the API key in the first step. 

2.  Based on the specific job roles or technical skills expected in candidates, create and publish the DevPlatform Tests, which must be accessible in Lever to send Test invites to candidates.

    To know more about creating and publishing a test, refer to[📄 Creating a New Test](/articles/7073839065)

    \

**Note**: In DevPlatform for Work, you must log in using the same **Company Admin** user account to create the DevPlatform Tests and generate the Lever API token required to enable the integration between the two applications.

## Sending DevPlatform Test invites from candidate profiles in Lever

The integration automatically creates the **DevPlatform** assignment form for the relevant Job interview process stages in Lever, and you can assign this form to the required stages.
...
*The 'DevPlatform' assignment form for different interview stages in Lever*

Simply change the candidate pipeline stage to the one that is assigned the **DevPlatform** form, and you will find the **Send Test** option to invite the candidate to a DevPlatform Test.
...
**Note**: A candidate's Lever profile must have a valid email address to send a DevPlatform Test invite.

In the Send test window,

1.  Click on the **Choose a Test** option and select the required DevPlatform Test. 
...
4.  Click on the **Send Test** to send the DevPlatform Test email invite.
...
*Sending a DevPlatform Test invite in Lever*
...
*The Candidate's DevPlatform Test report is updated in Lever*
...
Please refer to the [Lever support article](https://help.lever.co/hc/en-us/articles/217332106-How-do-I-enable-and-use-the-DevPlatform-Integration-) to know how Lever users can view and receive alerts about their candidates' performance in DevPlatform Tests.
