...
source_url: "https://support.devplatform.com/articles/9868451767-adding-candidates-to-lever-ats-from-a-public-link"
...
<tbody>
<tr>
<td data-colspan="1" data-rowspan="1" style="background-color: #eafcef; width: 453.477px; height: 22px"><p><strong>DevPlatform for Work</strong></p></td>
...
1.  Log in to your DevPlatform for Work account.
...
3.  Ensure that Lever has been integrated with DevPlatform Tests: 

    - If your Lever Integration(s) need to be set up, refer to the[📄 Lever - DevPlatform Tests Integration](/articles/7177093032) article.
...
1.  Then, within **Lever**, you'll need to access **Settings** and click on the Integrations and API tab, to ensure that the DevPlatform Tests integration is connected. 
...
2.  Mapping a job from the ATS to the test can fail under the following circumstances:
    ...
    - **The job has no owner** - You can see a prompt screen where you can assign an appropriate DevPlatform for Work user as the owner for the job.

    - **The owner of the job does not have a DevPlatform for Work account** - You can see a prompt to create a user account.

      - **The owner of the job does not have test access** - You can see a prompt to add the recruiter to a team on DevPlatform for Work, thereby granting them test access.
