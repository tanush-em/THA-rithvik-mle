---
title: "Lever - DevPlatform Interview Integration Overview Lever - DevPlatform Interview Integration"
title_slug: "lever-devplatform-interview-integration-overview-lever-devplatform-interview-integration"
source_url: "https://support.devplatform.com/articles/8274807044-lever---devplatform-interview-integration"
article_slug: "8274807044-lever---devplatform-interview-integration"
...
# Lever - DevPlatform Interview Integration Overview Lever - DevPlatform Interview Integration

_Last updated: Dec 28, 2024, 4:06 AM (Last updated 1 year ago)_

# Overview

DevPlatform Interviews integrates with Lever to facilitate a seamless hiring process for companies. The Interview is an efficient tool for recruiters and technical managers to conduct interactive coding interviews with candidates located remotely. The integration with interview allows Lever users to directly start or schedule coding interviews with their candidates.

This article provides you with detailed steps to integrate **Lever** with **DevPlatform Interview**. To know how to use the integration and schedule interviews from Lever, see[📄 Scheduling DevPlatform Interviews with Candidates in Lever](/articles/2634152447).

# Lever - DevPlatform Interview Integration
...
<tbody>
<tr>
<td data-colspan="1" data-rowspan="1" style="background-color: #eafcef; width: 49.8571%"><p><strong>In DevPlatform for Work</strong></p></td>
...
Lever - DevPlatform Interview integration is a two-step process that involves the following actions.

- Obtaining Webhook signing token and API key from Lever

- Obtaining webhook URL from DevPlatform and connect it to Lever
...
1.  Copy the token and store it as it needs to be added to the DevPlatform. 
...
*This page provides settings to enable integration with DevPlatform*

1.  Scroll down on the page to the **Assessment** section, and enable the **DevPlatform CodePair** option. \
    **Note: **Although you'll need to enable the DevPlatform CodePair, **you will not need to copy the key that is generated**.
...
*Enabling Lever's integration with DevPlatform Interview*

1.  After checking on the connection status of DevPlatform Tests, scroll up to the top of the page and access the **API Credentials Tab **(At the top of the screen)
...
2.  From the **API Credentials t**ab, scroll down to the **Lever API credentials** section**.** (As seen highlighted above within the API Credentials tab)
    ...
    - You'll then need to ensure that the Key that you'll be working with for **DevPlatform Tests** and **DevPlatform Interviews** has the proper **Read **and **Write endpoints** assigned, including both:
...
### Obtaining the Webhook URL from DevPlatform and Adding it in Lever
...
*Accessing the settings page in DevPlatform for Work*
...
*The DevPlatform settings page with Lever integration fields*

1.  DevPlatform creates the Webhook URL. Copy this URL. The integration completes when you add this URL to Lever.
    ...
    *The Webhook URL from the DevPlatform settings page*
...
1.  Click on the **Add Webhook** option for the **Candidate Stage Change** event, and add the Webhook URL obtained from DevPlatform.
...
2.  Click on the **Verify Connection** button. When the URL is verified, you will see a checkmark displayed. You can include or remove the 'https://' from the DevPlatform URL.
...
3.  \
    ...
    *Adding DevPlatform's Webhook URL in Lever*\
    \

In Lever, candidate profiles must meet the following requirements before the "**Candidate stage change**" event can trigger a request to DevPlatform Interviews:

1.  A profile must have the candidate's name and a valid email address.

2.  A profile must include the "**DevPlatform CodePair"** tag or be linked with a **Job** that has the same tag.
...
For more information, see[📄 Scheduling DevPlatform Interviews with Candidates in Lever](/articles/2634152447)
...
*Candidate profile in Lever with the "DevPlatform Interview" tag*
