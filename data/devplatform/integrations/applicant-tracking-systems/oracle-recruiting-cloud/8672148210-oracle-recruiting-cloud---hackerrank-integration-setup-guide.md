---
title: "Oracle Recruiting Cloud - DevPlatform Integration Setup Guide"
title_slug: "oracle-recruiting-cloud-devplatform-integration-setup-guide"
source_url: "https://support.devplatform.com/articles/8672148210-oracle-recruiting-cloud---devplatform-integration-setup-guide"
article_slug: "8672148210-oracle-recruiting-cloud---devplatform-integration-setup-guide"
...
# Oracle Recruiting Cloud - DevPlatform Integration Setup Guide

_Last updated: Jan 29, 2026, 9:24 PM (Last updated 3 months ago)_

## Overview

DevPlatform integrates with Oracle Recruiting Cloud (ORC) for Tests and Interviews to facilitate an easy and seamless hiring experience. The purpose of this document is to provide a step-by-step guide for setting up the Oracle Recruiting Cloud (ORC) - DevPlatform Integration.
...
We recommend setting up the integration in a sandbox ORC and DevPlatform instance before implementing it in your production environment.

### **Prerequisites**

User has Company Administrator access on DevPlatform\
Recruiting Administrator role (ORA_PER_RECRUITING_ADMINISTRATOR_JOB) on ORC

## Integration Setup

1.  Log in to DevPlatform with an Administrator license.

2.  Navigate to Settings\>Integrations\>[Oracle Recruiting Cloud.](https://www.devplatform.com/work/settings/api/oraclerecruitingcloud)
...
8.  Create an API User for DevPlatform on ORC.
    ...
    - **Optional**:
      ...
      - We also support the addition of IP allowlisting for an extra layer of security. This ensures that only requests coming from DevPlatform’s servers make it through to your ORC instance.

      - If your API User Name and Password are compromised, this logic will block requests generated from servers outside DevPlatform.
...
10. Generate OAuth Tokens from DevPlatform:

    - For this integration, DevPlatform uses OAuth 2.0 to authenticate incoming API requests.
    ...
    - If you misplace the token and cannot find it to be entered in ORC, you can come back and regenerate tokens from DevPlatform. One thing to remember is that if your integration is active with any old tokens, it will stop working if you delete these tokens from DevPlatform.

11. Find the **DevPlatform Integration Provisioning** page in ORC:

    - In ORC, you need to enter all the OAuth tokens and activate the integration.

    - You need to get to the DevPlatform Integration setup page. Follow these steps to get there:
    ...
    - In the Integration Categories under Assessment Partners, you should see DevPlatform. Click on the Edit (pencil) icon placed at the right end.
    ...
    - You will land on the DevPlatform Integration page.
...
12. Enter Integration Details in ORC:
    ...
    - Once you get to the Integration Provisioning page, enter the 3 tokens generated from DevPlatform: Reference Key, Client ID, and Client Secret.
...
13. Assign User Account in ORC:
    ...
    - This is the API User to which all DevPlatform Tests and interviews will be tied.\
      In the modal, enter the User Name as ORC_DEVPLATFORM_USER (Use all caps and no spaces).
    ...
    - Once you click on OK, ORC sends us this User Name and initiates the first API call made from ORC to DevPlatform.

    - At this stage, ORC asks DevPlatform for a list of all tests and interviews accessible to your company. We respond with 5 Interviews and up to 494 of your most recent Tests. (That makes it a total of 499 since ORC does not accept more than 499 packages for an integration).\
      If you are able to see the list of tests shown in Screening Packages, then the integration was successful.

    - If you are unable to see the tests or if ORC returns an invalid User Name error, check if you entered the “ORC_DEVPLATFORM_USER” as is without spaces or quotes.
...
14. Assign Additional User Accounts in Oracle Recruiting:
    ...
    - DevPlatform supports adding more than one user account on Oracle. In the previous step, you added "ORC_DEVPLATFORM_USER" which is a user account that pulls in all the available DevPlatform tests from your DevPlatform account.

    - DevPlatform, now, supports adding individual users as user accounts on Oracle Recruiting. You can follow the same steps in Oracle by clicking on **+Assign User Account** and entering the users' email IDs.
    ...
    - Note that these email IDs need to match with a valid DevPlatform license.
    ...
    - By adding individual accounts, only the tests that the provided email ID has access to will be pulled into Oracle. Thus making it easier for users to select a test that is most relevant to them and avoiding scrolling through a list of tests for roles and regions that they are not a part of.\
      This feature supports recruiters across teams and geographies to use the DevPlatform - Oracle Recruiting integration effectively.

    - All the added user accounts will show up on the integration settings page in DevPlatform.
    ...
    - Note that if you want to delete these additional user accounts, you could do so from the integration settings page in DevPlatform.
...
15. Enter API User Details and ORC Base URL in DevPlatform:

    - Open DevPlatform ORC Integration Settings Page.
...
Now, your integration is successful and activated. You and users in your organization will be able to use DevPlatform from within ORC.\
\
