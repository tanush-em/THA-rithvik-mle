---
title: "Oracle Recruiting Cloud and DevPlatform Integration User Guide"
title_slug: "oracle-recruiting-cloud-and-devplatform-integration-user-guide"
source_url: "https://support.devplatform.com/articles/3350882088-oracle-recruiting-cloud-and-devplatform-integration-user-guide"
article_slug: "3350882088-oracle-recruiting-cloud-and-devplatform-integration-user-guide"
...
# Oracle Recruiting Cloud and DevPlatform Integration User Guide

_Last updated: Dec 28, 2024, 4:37 AM (Last updated 1 year ago)_

DevPlatform integrates with Oracle Recruiting Cloud for Tests and Interviews to facilitate an easy and seamless hiring experience. This document walks through different use cases supported by the DevPlatform—Oracle Recruiting Cloud (ORC) Integration and covers FAQs.
...
- Generate DevPlatform Interview links for Virtual & Remote Onsite Interviews with candidates.
...
1.  DevPlatform - ORC Integration Setup is completed

2.  The user has access to a DevPlatform license

3.  The user has access to requisitions on ORC

## Types of Assessment Triggers supported

Oracle Recruiting Cloud offers various assessment triggers that can be configured to send assessments to candidates. DevPlatform has built the integration to support all these configurations that users can set up in ORC.
...
- **External Applyflow Trigger:** When this trigger is set up, candidates will be asked to take the assessment right after completing the job application on their ORC application profile. We recommend using this trigger type only for DevPlatform Tests. At this stage, a live DevPlatform Interview would not be logical.
...
In addition to the message shown by ORC, we also send candidates an email invite. This will use your default DevPlatform email settings.\
...
When this trigger is set up, candidates will be asked to complete an assessment while filling out the job application and before submitting it. We recommend using this trigger type only for DevPlatform Tests. At this stage, a live DevPlatform Interview would not be a logical assessment.
...
When the candidates click on the **Take Assessment** button, ORC redirects them to DevPlatform to complete the assessment.
...
## Types of DevPlatform Assessments

The DevPlatform platform supports two types of assessments. Both these types of assessments are supported via integration with ORC.
...
- **DevPlatform Tests**
  ...
  - For DevPlatform Test invites from Oracle, we send email invites to candidates with a link to take the assessment, except for the External Applyflow Inline trigger type.

<!-- -->

- **DevPlatform Interviews**

  - Includes synchronous interviews with single or multiple interviewers.

  - For DevPlatform Interview links generated from ORC, we do not send email notifications to candidates. We send the links back to ORC, and recruiters or coordinators can share this link with candidates and interviewers.

## Adding Assessments to a Requisition

Once the integration is set up and your team has defined the assessments for a job, you should be able to add DevPlatform tests and interviews to an ORC requisition.
...
- Select **DevPlatform** as the assessment partner and choose the relevant user account.

- If you have a specific user account provisioned, you could use that. Otherwise, select **ORC_DEVPLATFORM_USER** as the user account.

- If you select a specific user email under the user account, then you can only choose a DevPlatform test that the user can access. For **ORC_DEVPLATFORM_USER**, you can select any DevPlatform test in your company's account.
...
- Users can also choose a single Phase and State to trigger one assessment for the candidate. The Phase and State can be selected from the drop-down, and users can map one DevPlatform assessment to it. Users can choose a DevPlatform Test or a DevPlatform Interview. This assessment will be triggered via the CSP flow when a candidate is moved to that Phase and State.

  - **For DevPlatform Tests:** Choose any test name from the drop-down menu. This test will be sent to the candidate when they are moved to the mapped Phase and State. Please ensure that the recruiter assigned to the requisition has access to the test on DevPlatform.

  - **For DevPlatform Interviews:** Users can generate a unique DevPlatform interview link in up to 5 interview stages. Note that multiple interviewers can use one link for the same candidate. Select one of the 5 DevPlatform Interview options for each stage where you must conduct a DevPlatform interview.

- Select DevPlatform as the assessment partner for the Multi-Phase Assessment Setup, choose an appropriate user account, and select the **Add Assessment Phases** option. Click **OK**. Then click on the edit (pencil) icon.
...
- Users can also choose one assessment to be triggered for each available Phase and State. Under each phase and state label, users can choose a unique DevPlatform Test or a DevPlatform Interview. The CSP flow will trigger these assessments when a candidate is moved to that Phase and State.
...
### Triggering DevPlatform Tests

If a DevPlatform Test is triggered via the CSP, candidates will receive an email from DevPlatform asking them to take the test.
...
If a DevPlatform Interview is requested via the CSP, DevPlatform generates a unique interview link and sends it back to the candidate’s ORC profile.
...
After the assessment (test or interview) is triggered, the candidate’s DevPlatform profile will also reflect the action. This will maintain a mapping of the user who requested the assessment from ORC.
...
The recruiter requesting assessments from ORC needs a DevPlatform license with the same email address.

## Reviewing Results and Evaluation

Once a candidate has completed their DevPlatform test or interview, their results will be available on DevPlatform and synced with ORC.

The feedback available for DevPlatform tests and interviews is different. Here is a screenshot of the candidate’s ORC profile before completing a DevPlatform assessment.
...
ORC provides six fields for DevPlatform to update:
...
- **View Results:** Button with a hyperlink to detailed DevPlatform reports

### DevPlatform Tests

After completing the DevPlatform test, candidates' test scores and percentiles will be sent back to ORC.
...
DevPlatform updates the provided fields as follows:
...
- **Band:** Upon completion, the band can be **Evaluated**, **Pass**, or **Fail**. By default, it will be set to **To Be Evaluated** upon assessment completion. If a cutoff score is set on the test level in DevPlatform, the band will automatically update to **Pass** or **Fail**.

- **Comments:** For DevPlatform tests, we provide additional test results in the comment section. This includes the **Max Score**, attempt end time and **Question Details**.

- **View Results:** Upon test completion, this link will direct to the most recently completed DevPlatform assessment report.

**Evaluating DevPlatform test results**
...
- Users can review candidates’ scores. For a detailed DevPlatform report, click the **View Results** button. This button will direct you to the candidate’s most recently completed DevPlatform assessment report.

- Users will require a DevPlatform license to view a candidate’s detailed report on DevPlatform.

- Once a user evaluates the candidate’s test submission and code, the user can mark the candidate as **Test Completed** - **Qualified** or **Test Completed - Failed** on DevPlatform.
...
- Based on the evaluation on DevPlatform, the ORC profile status for that particular test will update to **Completed** - **Pass** or **Completed** - **Fail**. The Band will also update to **Pass** or **Fail**.
...
Once a DevPlatform interview is triggered for the candidate, a unique link will be posted back to the candidate’s ORC profile. This interview link can be used to conduct interviews with the candidate.
...
- **Score:** This will remain blank or 0 for DevPlatform interviews.

- **Assessment Percentile:** It will remain blank or 0 for DevPlatform interviews.
...
- **Comments:** We provide additional details in the comment section for DevPlatform interviews. This includes the Title, Start Time, End Time, Interviewer Details, and Scorecard.

- **View Results:** Upon completion, this link will direct you to the most recently completed DevPlatform assessment report.

**Evaluating DevPlatform interview results**
...
- Users can review the interview feedback. For a detailed DevPlatform report, click the **View Results** button. This button will direct you to the candidate’s most recently completed DevPlatform assessment report.

- Users will require a DevPlatform license to view candidates’ detailed reports on DevPlatform.

- On the detailed DevPlatform interview report, users can see candidates’ live coding submissions, whiteboards, and feedback from interviewers.

- After reviewing the necessary information, users can choose the evaluation and mark the candidate with a **Thumbs Up** or **Thumbs Down** on DevPlatform.
...
- Based on the evaluation on DevPlatform, the ORC profile status for that particular interview will update to **Completed - Pass** or **Completed - Fail**. The Band will also update to **Pass** or **Fail**.
