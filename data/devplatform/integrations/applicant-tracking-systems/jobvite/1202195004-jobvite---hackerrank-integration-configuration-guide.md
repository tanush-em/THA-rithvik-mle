---
title: "Jobvite - DevPlatform Integration Configuration Guide"
title_slug: "jobvite-devplatform-integration-configuration-guide"
source_url: "https://support.devplatform.com/articles/1202195004-jobvite---devplatform-integration-configuration-guide"
article_slug: "1202195004-jobvite---devplatform-integration-configuration-guide"
...
# Jobvite - DevPlatform Integration Configuration Guide

_Last updated: Mar 11, 2026, 4:49 PM (Last updated 2 months ago)_

## Overview

DevPlatform Tests integrate with Jobvite to trigger DevPlatform Test invites directly from Jobvite. Recruiters managing candidate profiles in Jobvite can use this integration to carry out a seamless and efficient candidate skills assessment for different requisitions without leaving their Jobvite account. Candidates receive DevPlatform Test invites directly from Jobvite, and after they complete the test, their scores and performance data are automatically available for evaluation in their Jobvite profile.

This article describes the configuration steps required for integrating  DevPlatform with Jobvite.
...
<tbody>
<tr>
<td data-colspan="1" data-rowspan="1" style="background-color: #eafcef; width: 50%"><p><strong>DevPlatform</strong><br />
<br />
</p></td>
...
<li><p>Log in using the same Company Admin user credentials to create and publish DevPlatform Tests, which must be accessible by Jobvite.</p></li>
</ul></td>
...
The integration involves a two-step process, after which you must verify whether the DevPlatform assessment-related custom fields are available in Jobvite.

## Configuring the Integration for DevPlatform - Jobvite

These sections describe the steps to set up the integration on the two platforms.

- [Retrieving the API Key from Jobvite](https://support.devplatform.com/articles/1202195004-jobvite---devplatform-integration-configuration-guide#retrieving-the-api-key-from-jobvite-9)

- [Adding the API Key and Secret in DevPlatform for Work](https://support.devplatform.com/articles/1202195004-jobvite---devplatform-integration-configuration-guide#adding-the-api-key-and-secret-in-devplatform-for-work-11)

- [Configuring the DevPlatform Test ID in Jobvite](https://support.devplatform.com/articles/1202195004-jobvite---devplatform-integration-configuration-guide#configuring-the-devplatform-test-id-in-jobvite-13)

### Retrieving the API Key from Jobvite

You need to contact the Jobvite support to obtain the** API key** and **Secret** that are required for the integration with your DevPlatform for Work account.

### Adding the API Key and Secret in DevPlatform for Work
...
### Configuring the DevPlatform Test ID in Jobvite

In Jobvite, the integration creates DevPlatform assessments-related workflow steps in the *Candidate* object and adds custom fields in the *Candidate* and *Requisition* objects.

To validate the DevPlatform workflows in Jobvite:

1.  In the *Candidate* object, click on the **Workflows** drop-down list and verify that the DevPlatform Assessment workflows are available. There are three workflows:

    - DevPlatform Assessment: This assessment workflow indicates the DevPlatform Test status sent to the candidate.

    - DevPlatform Assessment Completed: This assessment workflow indicates the DevPlatform Test completion status of the candidate.

    - DevPlatform Assessment Error: This assessment workflow indicates that the test could not be completed by the candidate due to some technical glitches.\
      \
...
5.  Click the **Candidate** tab. Verify that the DevPlatform assessment-related custom fields are created for the Candidate object as shown below.\
    \
...
The following table summarises the custom fields for DevPlatform.

|  |  |
|----|----|
| **Candidate - Custom Fields** | **Description** |
| DevPlatform Test Link | The URL of the test sent to the candidate. This is useful if the recruiter wants to resend the invite to candidates manually |
| DevPlatform Test Status | A string indicating the status of the invite. If this is not empty, then it will be one of the following: “Invited” or “Completed” |
| DevPlatform Test Score | The score obtained by the candidate for that particular test |
| DevPlatform Test Report Link | The URL of the test report sent to the interviewer. This is useful if the recruiter wants to share the candidate's test performance with other team members |
| DevPlatform CodePair Link |   |

Verify that the **DevPlatform Test ID** custom field is created under the **Requisition** object.
...
| DevPlatform Test ID | Edit this field to add the required DevPlatform Test IDs. When you create or edit a particular Requisition, you select and assign one of the Test IDs from this field. |

**Note**: For each requisition in Jobvite, a DevPlatform Test is tied to the requisition.

To view the DevPlatform Test Id for the requisition, select the requisition and click the **Details** tab.
