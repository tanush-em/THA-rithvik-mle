---
title: "Jobvite - DevPlatform Integration User Guide"
title_slug: "jobvite-devplatform-integration-user-guide"
source_url: "https://support.devplatform.com/articles/2429924508-jobvite---devplatform-integration-user-guide"
article_slug: "2429924508-jobvite---devplatform-integration-user-guide"
...
# Jobvite - DevPlatform Integration User Guide

_Last updated: Mar 11, 2026, 9:40 PM (Last updated 2 months ago)_

## Overview

This article describes how you can use the integration to trigger DevPlatform Test invites for candidates from Jobvite.
...
## Configuring Settings to Send DevPlatform Test Invites From Jobvite

After integrating DevPlatform with Jobvite, you can send DevPlatform Assessments from Jobvite to candidates. The following sections guide you on the settings required for both platforms to send assessments to candidates.

- [Creating Specific Tests in DevPlatform for Jobvite Candidates](https://support.devplatform.com/articles/2429924508-jobvite---devplatform-integration-user-guide#creating-specific-tests-in-devplatform-for-jobvite-candidates-6)

- [Associating Requisitions with DevPlatform Test IDs](https://support.devplatform.com/articles/2429924508-jobvite---devplatform-integration-user-guide#associating-requisitions-with-devplatform-test-ids-9)

- [Triggering DevPlatform Test Invites to Candidates](https://support.devplatform.com/articles/2429924508-jobvite---devplatform-integration-user-guide#triggering-devplatform-test-invites-to-candidates-15)

### Creating Specific Tests in DevPlatform for Jobvite Candidates

Before using the integration to trigger DevPlatform Test invites from Jobvite, you must create and publish the required Tests in DevPlatform for Work.

1.  Log into **DevPlatform for Work** using your **Company Admin** user account.

2.  Create and publish the DevPlatform Tests based on the specific job roles or technical skills expected of the candidates. To know more about creating and publishing a test, refer to[📄 Creating a New Test](/articles/7073839065)
    \

3.  In Jobvite, you must associate the DevPlatform Test IDs with Requisitions. Therefore, note the IDs for the required DevPlatform Tests.
...
### Associating Requisitions with DevPlatform Test IDs

1.  On Jobvite, add the required Test IDs in the **DevPlatform Test ID** field of the *Requisition* object.

2.  Edit the **DevPlatform Test ID** custom field to include the Test IDs and click **Save**.
...
You can assign one of the above DevPlatform Test IDs to a Requisition. For candidate profiles associated with this Requisition, when the Workflow status is changed to *DevPlatform Assessment*, the event triggers invites for the DevPlatform Test.

1.  Edit a Requisition and in the **DevPlatform Test ID** field, select the ID of the required DevPlatform Test and click **Save**.

2.  Similarly, you can associate a particular DevPlatform Test ID with all other **Requisitions**.
...
1.  Link the candidate profiles with the relevant Requisitions containing the DevPlatform Test IDs.

### Triggering DevPlatform Test Invites to Candidates

1.  When the candidate is ready to take an assessment, change the **Candidate Workflow** status to *DevPlatform Assessment*.\
    \
...
3.  Once the test is completed, the workflow status in Jobvite changes to *DevPlatform Assessment* *Completed.*
...
The **DevPlatform Test Status** is set to error status when an invite cannot be sent or another such error exists. These are the possible error statuses and the resolutions:
...
<td data-colspan="1" data-rowspan="1" style="width: 314.375px"><p>Verify that the candidate's email address is valid. Add an alternate email address and try changing the workflow status to <em>DevPlatform Assessment</em></p></td>
</tr>
<tr>
...
<td data-colspan="1" data-rowspan="1" style="width: 314.375px"><p>In DevPlatform for Work, ensure that the Test ID linked with the Requisition exists and is a published Test.</p></td>
</tr>
<tr>
<td data-colspan="1" data-rowspan="1" style="width: 314.375px"><p><em>Not Invited. Company/Test Owner in DevPlatform does not have permission to invite candidates.</em></p>
<p><em>Not Invited. Company/Test Owner in DevPlatform does not have permission to read the test.</em></p></td>
<td data-colspan="1" data-rowspan="1" style="width: 314.375px"><p>In DevPlatform for Work, ensure that the Company Admin user account is set to the <strong>Recruiter</strong> type and has the required permissions to send invites.</p></td>
</tr>
<tr>
<td data-colspan="1" data-rowspan="1" style="width: 314.375px"><p><em>Not Invited. Inviting Owner's DevPlatform Account is not activated.</em></p></td>
<td data-colspan="1" data-rowspan="1" style="width: 314.375px"><p>Ensure that the DevPlatform for Work user account is activated.</p></td>
</tr>
</tbody>
</table>
...
### After the Test result is initially synched if I change the score in DevPlatform, will it be synched back to Jobvite?

Yes. The first time DevPlatform sends results to Jobvite is at the end of the test once the report is generated. After that, any user action in DevPlatform for Work that changes the candidate report will be synched back to Jobvite. The significant visible changes are:
...
### I moved a candidate to the DevPlatform Assessment state, but the candidate has not yet received the email. Why?

The DevPlatform servers pull the Jobvite servers once in every 60 minutes. Therefore, it can take an hour for the candidate to receive the invitation email.
...
### Can I use a custom template that is saved on DevPlatform?

You cannot save a custom template to be sent to the candidate. The DevPlatform for Work default template is used.
