---
title: "Workday - DevPlatform Tests Configuration Guide"
title_slug: "workday-devplatform-tests-configuration-guide"
source_url: "https://support.devplatform.com/articles/1782819445-workday---devplatform-tests-configuration-guide"
article_slug: "1782819445-workday---devplatform-tests-configuration-guide"
...
# Workday - DevPlatform Tests Configuration Guide

_Last updated: Mar 26, 2025, 11:41 AM (Last updated 1 year ago)_

## Overview

This article discusses the configurations required to set up DevPlatform and Workday integration.
...
3.  DevPlatform Configuration

## Workday Security Configuration

Login to your Workday account and follow the steps below to complete the security configuration for setting up the integration between Workday and DevPlatform.

1.  Access the **Create Integration System User** task and configure a Workday account for the integration.

    - Name the account “DevPlatform_Integration_User.”
...
1.  Create Integration System Security Groups (Unconstrained).
    ...
    - Name the Security Group “DevPlatform_Integration_SG”
...
- On the **Edit Integration System Security Group (Unconstrained)** screen, select the Integration System User account *User DevPlatform_Integration_User* to include in the security group.
...
You can add an <u>extra</u> layer of security by implementing an IP Allowlist. The DevPlatform service communicates to Workday from the following two IP addresses:
...
- Setup DevPlatform Assessment in Workday
...
| **DevPlatform Send Invite** | DevPlatform_Send_Invite | Yes |   |
| **DevPlatform Accepted** | DevPlatform_Accepted | Yes |   |
| **DevPlatform Declined** | DevPlatform_Declined | Yes |   |
| **DevPlatform Invited** | DevPlatform_Invited |   | Yes |
| **DevPlatform Completed - Qualified** | DevPlatform_Completed_Qualified |   | Yes |
| **DevPlatform Completed - Evaluation Required** | DevPlatform_Completed_Evaluation_Required |   | Yes |
| **DevPlatform Completed - Failed** | DevPlatform_Completed_Failed |   | Yes |
...
DevPlatform customers can create their own custom tests, and the identifiers for these tests are unique for each customer. Therefore, any test names and test IDs must be added to Workday on a per-customer basis. This process is not something that can be automated, so it requires an implementer to add tests manually to Workday.

**Steps:**

1.  Open a test in DevPlatform

2.  Note down the Name and Test ID from the DevPlatform Test page and the page URL, respectively. In the example below, the Test ID is 1128102, and the Test Name is “Software Engineer Hiring Test.”  
...
1.  Set the Reference ID to the *Test ID* from DevPlatform
...
Note: When building out and testing the integration for the first time, you can get the Name and Test ID from the DevPlatform Test page and the page URL, as described above.

When the integration is ready, you can log into your DevPlatform for Work account and do a one-time bulk export of all the tests that need to be entered into Workday.

Follow the below steps in your DevPlatform account to bulk export the available tests.
...
Note: You need to add all new tests that are created in DevPlatform and need to be used in Workday manually. 
...
## **DevPlatform Configuration**

The DevPlatform side configuration involves the URL and authentication setup.

- Login to DevPlatform with an account that has Company Admin permissions.
...
**Note:**  Once the recruiter invites a candidate to take a test, DevPlatform will not invite the candidate again to retake the same test or take a different test. If you want to change this setup and want your candidate to get another chance after a certain period of time, you may control the setting from the below option.
...
This marks the end of the configuration process. After this, you can invite candidates for DevPlatform assessments from Workday and view their results.

You can check the[📄 Workday - DevPlatform Tests User Guide](/articles/1999755553)to learn more about the workflow.
