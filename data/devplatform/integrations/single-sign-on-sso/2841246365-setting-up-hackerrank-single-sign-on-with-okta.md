---
title: "Set up DevPlatform Single Sign-On with Okta Prerequisites Setting up DevPlatform Single Sign-On with Okta"
title_slug: "set-up-devplatform-single-sign-on-with-okta-prerequisites-setting-up-devplatform-single-sign-on-with-okta"
source_url: "https://support.devplatform.com/articles/2841246365-setting-up-devplatform-single-sign-on-with-okta"
article_slug: "2841246365-setting-up-devplatform-single-sign-on-with-okta"
...
# Set up DevPlatform Single Sign-On with Okta Prerequisites Setting up DevPlatform Single Sign-On with Okta

_Last updated: Apr 10, 2026, 2:26 PM (Last updated 2 weeks ago)_

DevPlatform for Work supports single sign-on (SSO) using Okta as the identity provider. Okta lets users sign in once and access DevPlatform without entering credentials again.

This article explains how to configure and enable SSO between DevPlatform and Okta.

For more information about SSO, see [📄 Getting Started with Single Sign-On](/articles/4264962721).

**Note:** DevPlatform accepts only the user’s email address for SSO. During setup, define the SAML assertion to send the email address. SSO fails if DevPlatform receives any value other than an email address.
...
# Setting up DevPlatform Single Sign-On with Okta

To set up DevPlatform single sign-On with Okta:

## Step 1: Copy the SSO unique ID from DevPlatform
...
3.  Copy the **SSO unique ID** under the **Configure SSO** section.
    ...
    **Note**: You need this SSO unique ID in **Step 2: Add DevPlatform for Work app in Okta**.

## Step 2: Add DevPlatform for Work app in Okta
...
4.  Search for **DevPlatform for Work** and select it.
...
7.  In the **Sign-On** tab:
    ...
    3.  In the **SSO unique ID** field, paste the SSO unique ID you copied from DevPlatform for Work in Step 1.
...
9.  Copy the metadata URL from the **Sign-On** tab and open it in your browser to download the XML file.
    ...
    **Note**: You need this XML file in **Step 3: Upload metadata in DevPlatform**.

## Step 3: Upload metadata in DevPlatform

1.  Open your **DevPlatform for Work** account.
...
After the upload completes, DevPlatform displays the autogenerated metadata URL.
...
- Users can sign in to DevPlatform through Okta.

- When users enter their email address, DevPlatform redirects them to the Okta sign-in page.
