...
source_url: "https://support.devplatform.com/articles/3939437783-setting-up-scim-provisioning-with-okta"
...
DevPlatform enables you to set up provisioning through your SSO by configuring the SCIM protocol.
...
- **Push new Users**
  ...
  - New users created through Okta and assigned to DevPlatform for Work are created within the application.

<!-- -->

- **Push Profile Updates**

  - Updates made to a user’s profile within Okta will update the user’s profile in DevPlatform for Work.

<!-- -->

- **Import New Users**

  - Users created within DevPlatform for Work can be downloaded and turned into a User object in Okta.
...
- **Push Groups**
  ...
  - Groups are provisioned as Teams and members within those groups are mapped into the team on DevPlatform for Work.
...
**Note:**  DevPlatform accepts only the user's email address. In the setup, the user defines the SAML assertion with the necessary data. DevPlatform SSO fails if it receives anything other than an email address.

## Prerequisites

- Your organization has an Enterprise plan with DevPlatform.

- [Configure SSO using SAML](https://saml-doc.okta.com/SAML_Docs/How-to-Configure-SAML-2.0-for-DevPlatform.html) before setting up the provisioning with DevPlatform for Work

## Configuring SCIM Provisioning with Okta

The process of configuring SCIM Provisioning with Okta includes the following:

1.  [Generating an API Key from DevPlatform](https://support.devplatform.com/articles/3939437783-setting-up-scim-provisioning-with-okta#generating-an-api-key-from-devplatform-18)

2.  [Configuring Provisioning for DevPlatform](https://support.devplatform.com/articles/3939437783-setting-up-scim-provisioning-with-okta#configuring-provisioning-for-devplatform-20)

3.  [Importing Users](https://support.devplatform.com/articles/3939437783-setting-up-scim-provisioning-with-okta#importing-users-23)

4.  [Provisioning Steps](https://support.devplatform.com/articles/3939437783-setting-up-scim-provisioning-with-okta#provisioning-steps-27)

### Generating an API Key from DevPlatform
...
### Configuring Provisioning for DevPlatform

1.  Click the **Provisioning** tab in the DevPlatform for Work application in Okta.
...
2.  Enter the API Token generated from DevPlatform for Work in **API Token**.
...
5.  Click **Save**. You can now begin provisioning users to DevPlatform for Work.

### Importing Users

The Import Users option helps to prevent overwriting existing permissions for users that are already created in DevPlatform. This option pulls users back into Okta to keep your directory in sync.

To import users:

1.  Click the **Import** tab in the DevPlatform for Work application in Okta.
...
2.  Click **Import Now** to pull the list of users from DevPlatform for Work.
...
4.  If users in DevPlatform for Work exist in Okta but are not assigned to the DevPlatform for Work application, an **EXACT** label is associated with the user. A new user is not created, but the user will be assigned to the application. 
...
Perform these steps to create users in DevPlatform for Work:

1.  Click the **Assignments** tab in the DevPlatform for Work application in Okta.
...
## Migrating to New Instance of DevPlatform

The DevPlatform SCIM application is updated to provide a better experience to Okta users. The summary of changes is:

- A new attribute (Interviewer Role) is added

If you are a current user, you need to add a new instance of DevPlatform in your Okta org.

If you already have an existing instance of DevPlatform, follow these steps to migrate to a newly updated instance of DevPlatform:
...
4.  Add a new instance of DevPlatform.
...
6.  After SCIM Provisioning has been enabled, go to the **Import** tab of your new DevPlatform app instance.
...
8.  After the users from DevPlatform are downloaded, select the users you want created or linked in Okta.
...
12. Open your previous DevPlatform app instance.
...
17. You can now deactivate or delete your previous DevPlatform app instance and use the new DevPlatform app you added.

**Note:**

- If you were using SAML as the sign-on mode for your previous DevPlatform app instance, you will need to set up SAML on your new DevPlatform app instance in Okta (recommended) or maintain the previous DevPlatform app instance to ensure that the SAML functionality continues to work.

- If you were using your previous DevPlatform app as a profile master for certain Okta attributes, you must set your new DevPlatform app as the profile master for the same attributes. 

### Push Groups

To have access to tests and candidate reports, it is a best practice for users to be part of at least one team.

Follow these steps to assign your users to a team within DevPlatform for Work.

**Warning**: Users must be provisioned to the application using Assignments for Push Groups to work. Group names must be unique and have not been created on DevPlatform.

1.  Click the **Push Groups** tab in the DevPlatform for Work application on Okta.
...
- If you want DevPlatform users to be in one team and have equal access to all tests and candidates, push a single group that all users belong to. For example, *Everyone* or *DevPlatform Users*.

- The group will be pushed and only those users assigned through the **Assignments** tab will be added to the team *Everyone*.

- DevPlatform users are in teams according to organization recruiting.

- If you want to create Teams in DevPlatform that correspond to organizations, divisions, or geo-locations, find the corresponding groups in Okta and push them as Teams in DevPlatform.

## User Attributes

There are various attributes that you need to configure when assigning or updating users.

- **Username/Email**: The primary email is used as the username in DevPlatform and cannot be updated from Okta. Choosing to update either username or primary email in Okta will not be reflected in the DevPlatform for Work application.
...
- This integration is optimized for creating users and teams. The following Okta features are not supported.
  ...
  - Teams in DevPlatform cannot be synced back to Okta as a Push Group option.
