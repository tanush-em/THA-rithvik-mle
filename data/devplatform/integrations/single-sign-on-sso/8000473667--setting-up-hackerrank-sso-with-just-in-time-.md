---
title: "Just-in-Time (JIT) Provisioning for DevPlatform SSO"
title_slug: "just-in-time-jit-provisioning-for-devplatform-sso"
source_url: "https://support.devplatform.com/articles/8000473667--setting-up-devplatform-sso-with-just-in-time-"
article_slug: "8000473667--setting-up-devplatform-sso-with-just-in-time-"
...
# Just-in-Time (JIT) Provisioning for DevPlatform SSO

_Last updated: Mar 25, 2026, 2:37 PM (Last updated 1 month ago)_

Just-in-Time (JIT) provisioning allows organizations that use Single Sign-On (SSO) to automatically create and manage user accounts in DevPlatform. This feature removes the need to manually preconfigure accounts. When JIT provisioning is enabled, users who sign in with SSO for the first time will automatically have a DevPlatform account created and ensures secure access management and a smooth onboarding experience. JIT provisioning does not trigger for non-SSO logins.

## Key benefits

JIT provisioning for DevPlatform SSO provides the following benefits:

- **Automated account creation:** Users are granted immediate access to DevPlatform upon authenticating via SSO, eliminating the need for IT administrators to manually create user accounts.
...
The following steps describe how JIT provisioning functions in DevPlatform:

1.  **User initiates login:** The user navigates to DevPlatform and selects the SSO login option.

    **Note:** JIT provisioning runs only during SSO login. If users log in without SSO, DevPlatform does not create user accounts automatically.
...
3.  **SSO authentication:** After successful verification, the IdP sends a SAML assertion with user attributes to DevPlatform.

4.  **Automatic account creation:** If JIT provisioning is enabled and the user does not already have an account, DevPlatform creates one automatically using the attributes provided by the IdP.

5.  **Access granted:** The user is logged in to DevPlatform and can begin using the platform immediately.

## Prerequisites

- Your organization has an Enterprise plan with DevPlatform.

- You must have an active DevPlatform for Work account with Company admin access.

- You must [configure SSO using SAML](https://saml-doc.okta.com/SAML_Docs/How-to-Configure-SAML-2.0-for-DevPlatform.html) before you use JIT provisioning.

## Enabling JIT provisioning in DevPlatform

To enable JIT provisioning:

1.  Log in to DevPlatform using a Company Admin account.

2.  Go to the **Settings \>** **Single Sign On**.

3.  In the **Just-in-Time provisioning** section, enter the email domain(s) that should be routed through SSO. For example, [devplatform.com](http://devplatform.com).
...
2.  Create or configure a SAML 2.0 application for DevPlatform, and set up the service provider (SP) details.
...
4.  Enable JIT provisioning in DevPlatform and verify that AWS IAM Identity Center is sending the correct attributes.
...
6.  Enable JIT provisioning in DevPlatform and verify that Azure AD is sending the correct attributes.
...
2.  Go to **Apps \> SAML Apps** and add DevPlatform as a new SAML application.
...
5.  Enable JIT provisioning in DevPlatform and verify successful sign-in using the mapped attributes.
...
If you need further assistance, contact us at [support@devplatform.com](mailto:support@devplatform.com).
