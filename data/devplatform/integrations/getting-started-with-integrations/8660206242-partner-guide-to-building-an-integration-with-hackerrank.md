---
title: "Partner Guide to building an integration with DevPlatform"
title_slug: "partner-guide-to-building-an-integration-with-devplatform"
source_url: "https://support.devplatform.com/articles/8660206242-partner-guide-to-building-an-integration-with-devplatform"
article_slug: "8660206242-partner-guide-to-building-an-integration-with-devplatform"
...
# Partner Guide to building an integration with DevPlatform

_Last updated: Feb 7, 2026, 3:34 PM (Last updated 3 months ago)_

This document is meant for partners who wish to develop an integration with DevPlatform for Work. We are constantly working with ATS vendors to develop integrations for a better user experience. This document will give you all the information you need to develop an integration yourself.

## Background

A number of our enterprise customers - i.e those who use DevPlatform for Work Tests and Interviews for the evaluation phase, also use some other IT systems to manage their overall hiring process. These systems could either be Commercial Off-the-shelf (COTS) systems such as Oracle Taleo, Jobvite, Greenhouse, Lever, Kenexa, RecruiterBox, etc., or some bespoke home-grown system. We will refer to these apps jointly as Applicant Tracking Systems (ATS).

Such customers frequently ask for integration between their ATS and DevPlatform for Work so that routine actions of managing their candidates can be done inside their ATS workflow. Examples of such routine actions include (a) inviting a shortlisted candidate to take a DevPlatform for Work test, (b) viewing the results of a test for such candidates in their ATS, (c) scheduling a DevPlatform interview session between an engineer and a selected candidate, (d) viewing Interview reports within the ATS interface, etc. Thus, the most commonly requested integration points involve pulling data from an existing DevPlatform for Work account and doing specific actions from inside the ATS interface. We have API calls that will help implement these actions from within the ATS UI.

## General Workflow

The integration typically involves adding functionality to the ATS via a plugin or customization using the DevPlatform for Work API. This is best explained visually using the following diagrams.
...
**Note:** Every official ATS integration will have a section within DevPlatform for Work where each company’s account admin will be able to generate a key. Your ATS will appear in the following location: [https://www.devplatform.com/work/settings/api](https://www.devplatform.com/work/settings/api) 
...
Head to [https://www.devplatform.com/work/signup](https://www.devplatform.com/work/signup) to sign up for a free 14 day trial of our product. This account will be required in order to explore our API (see below) and to test your integration.

### **Explore our API**

We have a simple RESTful API that will form the basis for the integration. You should start learning about our API here:

[https://www.devplatform.com/work/apidocs](https://www.devplatform.com/work/apidocs) 

**Note:** The above documentation is written with our end customers in mind. You should explore the API in your test account as an end customer. If you have any questions about the API you should reach out to your DevPlatform point of contact or raise a support request by writing to [support@devplatform.com](mailto:support@devplatform.com).

### **Register your Integration**

After you familiarize yourself with the API and mapped out the flows you want to support, contact your DevPlatform point of contact or email [support@devplatform.com](mailto:support@devplatform.com) to request a **partner key**. DevPlatform also issues a company-wide API key to use with your integration.
...
2.  **Add the user email header:** Include the following custom header:
    ...
    This email must match an existing user in the customer’s DevPlatform for Work account.
...
4.  With every call you make you may also choose to **include some additional metadata** in your payload if there is metadata you need DevPlatform to save. Some commonly seen fields include
...
Your integration will be available as an option on our Integration Settings Page: [https://www.devplatform.com/work/settings/api](https://www.devplatform.com/work/settings/api) 
...
1.  The API Key is not valid for your DevPlatform for Work account. (Needs to be the per-company Key and the Partner-authorization should be working properly)

2.  The recruiter's email address for the ATS account (sent via the metadata) is different than the email used within DevPlatform for Work (e.g. using sriram.karra@devplatform.com in the ATS account and sriram@devplatform.com in your HRW accounts). It does not matter which one you fix as long as they are the same.

3.  Candidate email address is missing or invalid

4.  Candidate with this email has already been invited.

5.  Recruiter does not have a "Recruitment Seat" on DevPlatform, and as such does not have the privilege to invite candidates

6.  Recruiter does not have permission to access a particular test

7.  Recruiter's DevPlatform Account is not activated.
...
    "username": “error@devplatform.com",
...
    **"error_message": "Candidate has already been invited to take the same test. If you want to reinvite, cancel the invite in your DevPlatform for Work account first."**
